...
from app.core.exceptions_new import SystemError, UnauthorizedError
...
def require_sdk_auth():
    ...
    Raises:
        UnauthorizedError: 当认证失败时抛出相应的错误
        SystemError: 当系统错误时抛出相应的错误
    """
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            ...
            if not request:
                raise SystemError("Request object not found")
...
def admin_required(func):
    ...
    Raises:
        ForbiddenError: 当用户不是管理员时抛出403权限错误
    """
    @wraps(func)
    async def wrapper(*args, **kwargs):
        from app.services.auth import get_current_admin_user
        from app.core.exceptions_new import ForbiddenError
...
def require_knowledge_base_permission(required_permission: PermissionType):
    ...
    Raises:
        ForbiddenError: 当用户没有足够权限时抛出 403 错误
        SystemError: 当系统错误时抛出相应的错误
    """
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            from app.core.exceptions_new import ForbiddenError, SystemError
